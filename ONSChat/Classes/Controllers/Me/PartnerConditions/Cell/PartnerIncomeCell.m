...
@implementation PartnerIncomeCell
...
- (void)awakeFromNib {
    ...
    NSString *str = [KKSharedLocalPlistManager kkStringForKey:Plist_Key_PartnerIncome];
    if (str.length>0) {
    ...
    self.bgView = bgView;
    NSMutableArray *incomeArr = [NSMutableArray arrayWithArray:KKSharedGlobalManager.incomeArr];
    [incomeArr insertObject:@"不限" atIndex:0];
    //第一个选项的Y值
    CGFloat startY = 1;
    CGFloat btnW = (KKScreenWidth-Interval)/2.0;
    CGFloat btnY = 0.0;
    for (int i = 0; i<incomeArr.count; i++) {
        NSString *optionStr = [incomeArr objectAtIndex:i];
...
-(void)savePartnerConditions{
    KKLog(@"收入 %@",self.selectedInfoLab.text);
    [KKSharedLocalPlistManager setKKValue:self.selectedInfoLab.text forKey:Plist_Key_PartnerIncome];
