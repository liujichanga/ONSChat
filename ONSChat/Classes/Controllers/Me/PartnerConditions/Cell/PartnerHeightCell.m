...
- (void)awakeFromNib {
    ...
    NSString *str = [KKSharedLocalPlistManager kkStringForKey:Plist_Key_PartnerHeight];
    if (str.length>0) {
    ...
    CGFloat btnY = 0.0;
    for (int i = 0; i<KKSharedGlobalManager.friendHeightArr.count; i++) {
        NSString *optionStr = [KKSharedGlobalManager.friendHeightArr objectAtIndex:i];
...
-(void)savePartnerConditions{
    ...
    KKLog(@"身高 %@",self.selectedInfoLab.text);
    [KKSharedLocalPlistManager setKKValue:self.selectedInfoLab.text forKey:Plist_Key_PartnerHeight];
