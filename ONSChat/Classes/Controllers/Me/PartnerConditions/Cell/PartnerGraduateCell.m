...
- (void)awakeFromNib {
    ...
    NSString *str = [KKSharedLocalPlistManager kkStringForKey:Plist_Key_PartnerGraduate];
    if (str.length>0) {
    ...
    CGFloat btnY = 0.0;
    for (int i = 0; i<KKSharedGlobalManager.graduateArr.count; i++) {
        NSString *optionStr = [KKSharedGlobalManager.graduateArr objectAtIndex:i];
...
-(void)savePartnerConditions{
    ...
    KKLog(@"学历 %@",self.selectedInfoLab.text);
    [KKSharedLocalPlistManager setKKValue:self.selectedInfoLab.text forKey:Plist_Key_PartnerGraduate];
